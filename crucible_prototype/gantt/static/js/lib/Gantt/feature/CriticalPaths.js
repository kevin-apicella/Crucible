    ...
    setupObserver() {
        ...
        me.criticalPathObserver = project.getGraph().observe(function * () {
            ...
            if (!me.disabled) {
                ...
                me.criticalDependencies = new Set();

                // check if dependencies feature is there
                if ((dependencies = dependencies || me.getDependenciesFeature())) {
                    for (const path of criticalPaths) {
                        for (const { dependency } of path) {
                            if (dependency) {
                                dependencies.highlight(dependency, me.criticalDependencyCls);
                                me.criticalDependencies.add(dependency);
                            }
                        }
                    }
                }
