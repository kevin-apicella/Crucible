...
export default class DependencyColumn extends Delayable(Column) {
    ...
    async finalizeCellEdit({ grid, record, inputField, value, oldValue, editorContext }) {
        ...
        if (record && value) {
            ...
            let valid = results.every(depIsValid);

            if (valid && results.length > 1) {
                const
                    depMap = {},
                    field = this.field === 'predecessors' ? 'from' : 'to';

                toValidate.map(dep => {
                    depMap[dep[field]] = depMap[dep[field]] || 0;
                    depMap[dep[field]]++;
                });
                valid = Object.values(depMap).every(val => val === 1);
            }
