...
import DomHelper, { isVisible, hasLayout, getParent } from '../helper/DomHelper.js';
...
const
    ...
    getLayoutTarget         = el => DomHelper.getStyleValue(el, 'display') === 'contents' ? getLayoutTarget(el.parentNode) : el,
    getEventTarget          = t => {
        if (t?.target) {
            let { offsetX, offsetY } = t,
                target               = getLayoutTarget(t.target);

            // Events targeted at display:contents elements report offsetX,offsetY as clientX,clientY
 ...
 * @property {Number|Number[]} [offset] The `[x, y]` position offset to create an extra margin round the target to offset
 * the aligned widget away from the target. May be configured as `-ve` to move the aligned widget towards the target - for example
 * producing the effect of the anchor pointer piercing the target.
 * When `offset` is configured as `Number` both `x` and `y` are equal to the provided value.
 * Offset is applied according to the used `align` value for the target element:
 * * `t`, `b` - `[y]` offset is used to update `y` target coordinate
 * * `r`, `l` - `[x]` offset is used to update `x` target coordinate
 * * `c` - `offset` is not used
...
/**
 * A context object injected into all cell widgets owned by grid widget columns. It allows event handlers
 * for the widget to ascertain the context in which they are operating.
 *
 * @typedef {Object} CellWidgetContext
 * @property {HTMLElement} cellElement The cell in which the widget is rendered.
 * @property {Core.data.Model} record The record backing the row into which the widget is rendered.
 */

/**
 * The base class for all other widgets in the Bryntum libraries. The Widget base class encapsulates a native DOM
 * element, and can optionally contain {@link #config-html}. See also {@link Core.widget.Container} which lets you
 * build a component tree where widgets can contain other widgets. Simple widget example (inspect with the DOM inspector
 * to see it's really just a simple DIV):
 *
 * {@inlineexample Core/widget/Widget.js}
 *
 * ## Rendering
 *
...
export default class Widget extends Base.mixin(Localizable, Events, Delayable, Identifiable, Factoryable, KeyMap, RTL) {
     ...
     * @static
     */
    static convertPinchToMousewheel = true;
    ...
    finalizeInit() {
        const
            me         = this,
            { adopt }  = me,
            refElement = me.insertBefore || me.appendTo || me.insertFirst || adopt;

        // Ingest lazy content configs
        me.getConfig('html');
    ...
    changeElement(element) {
        ...
        // Ensure we have an id. The changer will generate one if we were not configured with one.
        me.id = me.id || adopt?.id || null;

        // Ingest the adopt element for use as our encapsulating element
        if (adopt) {
    ...
    updateElement(element) {
        ...
        // If we have adopted an element, isVisible tests must still yield false,
        // so we flag ourself as hidden. Ensure that a composed element structure is present.
        // We will lift the hidden state in finalizeInit where auto-rendering takes place.
        if (me.adopt) {
            me._hidden = true;
            compose && me.recomposeNow();
        }

        // Add the listeners added by any composers
    ...
    render(renderTo, triggerPaint = true) {
        ...
        if (element.parentElement?.closest('.b-widget')) {
    ...
    alignTo(spec) {
        ...
        const
            ...
            targetEvent       = spec.realignEvent || getEventTarget(spec.domEvent || spec.event || (passedTarget?.target?.nodeType === Node.ELEMENT_NODE ? passedTarget : null)),
            aligningToEvent   = targetEvent?.target?.nodeType === Element.ELEMENT_NODE,
            target            = aligningToEvent ? targetEvent.target : passedTarget?.isRectangle ? passedTarget : passedTarget?.element || passedTarget,
        ...
        // Aligning to an event is just aligning to the event target's position plus the event's offsetX/Y
        if (aligningToEvent) {
        ...
        if (aligningToElement) {
            // anchoredTo is the element which determines our position.
            ...
            if (hasLayout(target instanceof SVGElement ? target.closest('svg') : target)) {
                ...
                spec.target = me.lastAlignSpec.targetRect =
                    spec.realignTarget || (spec.allowTargetOut ? Rectangle.from(target, positioned ? offsetParent : null, !positioned)
                        : DomHelper.isInView(aligningToEvent ? targetEvent : target, false, me));
        ...
        if (anchor?.edge) {
            const
                ...
                { style : svgStyle }    = anchorElement.firstChild,
        ...
        if ((me.scrollAction === 'realign' && aligningToElement || me.scrollAction === 'hide') && !me.documentScrollListener) {
            ...
            me.setTimeout(() => {
                ...
                // Realign if the main document detects a scroll or transition.
                ...
                // In case the align target is in a WC, also capture scroll and transitions scoped with in its shadow root
                if (targetRoot?.mode) {
    ...
    realign() {
        ...
        if ((me.floating || me.positioned) && lastAlignSpec && me.isVisible) {
            if (lastAlignSpec.aligningToElement) {
                ...
                // Unless we're inside the target, or the target is an event (eg contextmenu)
                // in which case the target itself is used.
    ...
    handleReactElement(html) {
        const
            parent = this.closest(cmp => cmp.reactComponent),
            // Attempt to find a React component. Fixes https://github.com/bryntum/support/issues/7964
            reactComponent = parent?.reactComponent || bryntum?.query(c => !!c.reactComponent?.state)?.reactComponent;
    ...
    updateHtml(html) {
        if (!this.initializingElement) {
            // When adopting, the html does not replace the initial content
    ...
    insertContent(newContent, replace = false) {
        const
            ...
            anchorEl = replace && (contentElement === element) && me._anchorElement;
        ...
        // Replacing content destroys the anchorElement in some browsers, we must temporarily
        // remove it to preserve it. Only if the contentElement is the main element.
        if (anchorEl) {
        ...
        // Removal of focused content should not cause an autoclose
        me.autoClose = false;
        ...
        if (newContent) {
            ...
            if (typeof newContent === 'object') {
                ...
                // this looks a bit weird, but this seems to fix the test
                // `content set via the content property #2` in `Core/tests/widget/Widget.t.js`
                // please feel free to apply a proper patch
                // motivation: https://github.com/bryntum/support/issues/8676
                if (!replace) {
                    contentRange.deleteContents();
                }
                // somehow, not clear why, the `contentRange` is collapsed after this call
                // so we need to clear the content _before_ it
                newContentFragment = DomHelper.createElementFromTemplate(newContent, { fragment : true });
            }

            // If it's not been taken care of by setting innerHTML
            if (!done) {
                if (replace) {
    ...
    get isVisible() {
        ...
        return Boolean(element && !me._hidden && !me.isDestroying && element.isConnected &&
            ...
            (!(owner = me.parent) || owner.isVisible)
        );
    }
    ...
    onChildShow(shown) {
        // If we actually *do* own it, connect it to us.
        if (shown.floating && this.childItems?.includes[shown]) {
    ...
    changeOwner(owner) {
        // The owner property must not be set if we are a child component of a Container
        // which injects a parent property.
        if (!this.parent) {
            return owner;
        }
    }
    ...
    eachWidget(fn, deep = true, itemsBeforeAux) {
        const
            widgets = itemsBeforeAux ? this.childItemsOrderedByRelevance : this.childItems,
    ...
    onFocusIn(e) {
         ...
         *
         * This means that either the DOM of this widget contains focus, __or, any widget DOM who's
         * {@link #property-owner} axis leads to this widget contains focus__.
         *
         * For example a {@link Core.widget.DateField} with its {@link Core.widget.DatePicker} open and
         * being interacted with will still have `containsFocus` set.
         * @member {Boolean} containsFocus
         * @readonly
         * @category Widget hierarchy
         */
    ...
    async executeAndAwaitAnimations(element, fn) {
        ...
        const
            animationsAfterFunction = element.getAnimations({ subtree : true }),
            animationsFromFunction  = ArrayHelper.delta(animationsBeforeFunction, animationsAfterFunction).onlyInB,
            // Avoid waiting for something like an infinite spinner animation to finish
            finiteAnimations        = animationsFromFunction.filter(animation => animation.effect.getTiming().iterations !== Infinity);

        // Wait for finite animations introduced by the function to complete
