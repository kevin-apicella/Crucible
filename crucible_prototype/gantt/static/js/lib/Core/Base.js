...
export default class Base {
    ...
    resolveProperty(propertyPath) {
        ...
        while (thisObj) {
            if (Base.hasConfigPath(thisObj, propertyPath, true)) {
    ...
    static hasConfigPath(object, path) {
        return path.split('.').every(key => {
            // We check for config presence if the property is a config, *and* there has not been
            // a new getter injected - the getter is the config's getter.
            const
                config   = object?.$meta?.configs?.[key],
                isConfig = config && config.descriptor.get === Objects.getPropertyDescriptor(object, key)?.get;

            // If it is a "configurable" object, we need to use hasConfig,
            // otherwise we just use "key in"
    ...
    getProperties() {
        ...
        for (let i = 0; i < hierarchy.length; i++) {
            ...
            // do not try to clone the properties defined with accessor: `static get properties () {...}`
            // this is because they return a fresh copy anyway and sometimes use very specific initialization for
            // properties, for example: `object : Object.create({})` in the DynamicObject,
            // which creates an object with the separate prototype
            Object.assign(result, desc.get !== undefined ? hierarchy[i].properties : Objects.clone(desc.value));
        }
